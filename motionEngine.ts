...
export class motionEngine {
    elements: Array<collisionBox> = []
    lastUpdate: number = Date.now()
    gravity: number
    PIXIContainer: any

    constructor(gravity: number, container: any) {
    ...
    #applyGravity(element: collisionBox): void {
        element.velocity.y += this.gravity
    }
    #applyVelocities(element: collisionBox, deltaTime: number): void {
        try {
            if (!element) {
    ...
    #updatePIXI(element: collisionBox, appWidth: number, appHeight: number): void {
        if (!element.pixi) {
    ...
    addPIXI(element: collisionBox, pixiElement: any, render?: boolean, color?: any): void {
        try {
    ...
    update(appWidth: number, appHeight: number): void {
        try {
     ...
     */
    createCollisionBox(screenX: number, screenY: number, polygon: polygon, position: { x: number, y: number }, options?: options): (collisionBox | undefined) {
        try {
    ...
    static arePolygonsColliding(A: collisionBox, B: collisionBox, tolerance = 0): boolean {
        return false
        /*
